...
namespace Drush\Commands\drupak;
...
class ThemeSetupCommands extends DrushCommands {
  ...
  /**
   * drupak theme setup command.
   */
  #[CLI\Command(name: 'drupak:setup', aliases: ['drupak'])]
  #[CLI\Usage(name: 'drush drupak:setup', description: 'Sets up the drupak theme requirements.')]
  ...
  public function createThemeSetup() {
    try {
      ...
      // Define paths.
      $themePath = $drupalRoot . '/themes/custom/drupak'; // Path to the theme directory.
      $ddevConfigPath = "$rootPath/.ddev/config.yaml";

      $this->logger()->notice("Setting up drupak theme...");
      ...
      $this->runCommand('drush then drupak -y; drush config-set system.theme default drupak -y', $rootPath);
      // Check if drupak is default theme
